...
(function ( $ ) {
	...
	$.widget("radioalbum.raSelector", {
		...
		_getMediaServerURL: function(){
			return this._getServerURL() + "/mserver/index.php?";
		},
        ...
        _getParamsConnectionURI: function(){
            return 'user='+self.connection.user+'&token='+self.connection.token+'&t='+self.connection.time;
        },
        
        _getAllowedActionURI: function(action){
            return self.actions[action].length ? self.controller+'/'+self.actions[action] : self.siteController+'/'+self.indexPage;
        },
		...
		_loadChannelsAsinc: function(){ 
			var self = this;
            let action = self._getAllowedActionURI('channels');
			var url = self._getMediaServerURL() + action + self. _getParamsConnectionURI;
			$.ajax({
				url: url,
			})
			.done(function(xml){
		...
		_initConnection: function(){
			...
			$.ajax({
				...
				success: function(data){
                    self.connection.t = data['time'];
                    self.connection.user = data['user'];
